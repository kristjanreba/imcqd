...
template <class Vertex> class Graph : public template_vector_container<Vertex> {
public:
  typedef set<Vertex *> VertexSet;
  typedef vector<Vertex *> Path;
  typedef vector<Path> Cliques;
  typedef set<VertexSet> SubGraphs;
  typedef pair<vector<node_id>, vector<node_id>> MatchedVertices;
  typedef vector<MatchedVertices> Matches;

private:
  vector<unique_ptr<Vertex>> __vertices; // if graph owns the vertices, they
                                         // (the unique_ptrs) are stored here
  AdjacencyMatrix __conn;
  vector<int> __num_edges;
  void __expand(Vertex &, Path &, SubGraphs &, VertexSet &);
  void __expand_components(Vertex &, VertexSet &, VertexSet &);
  template <class Vertex2>
  bool __match(vector<node_id> &, vector<node_id> &, Matches &,
               UllSubState<Graph<Vertex>, Graph<Vertex2>> *);
  template <typename T> void __init(const T &, const bool);

public:
  Graph() {}
  template <typename T> Graph(const T &vertices, const bool ict = false) {
    __init(vertices, ict);
  }
  ...
  SubGraphs find_components();
  SubGraphs find_cycles_connected_graph();
  SubGraphs find_fused_rings();
  SubGraphs find_rings();
  Cliques max_weight_clique(const int);
  template <class Vertex2> Matches match(Graph<Vertex2> &);
...
template <typename T> T intersection(T p1, T p2) {
  T v;
  auto it = set_intersection(p1.begin(), p1.end(), p2.begin(), p2.end(),
                             inserter(v, v.begin()));
  return v;
}
...
template <class Vertex>
typename Graph<Vertex>::Path
reconstruct_path(Vertex &goal, const map<Vertex *, Vertex *> &came_from) {
...
bool Graph<Vertex>::__match(vector<node_id> &c1, vector<node_id> &c2,
                            Matches &m,
                            UllSubState<Graph<Vertex>, Graph<Vertex2>> *s) {
  if (s->IsGoal()) {
    int n = s->CoreLen();
    s->GetCoreSet(c1, c2);
#ifndef NDEBUG
...
template <class Vertex>
typename Graph<Vertex>::Cliques
Graph<Vertex>::max_weight_clique(const int iter) {
  Benchmark::reset();
  ...
  clog << "time to find max.weight clique " << Benchmark::seconds_from_start()
...
//~ }
//~ clog << "time to find max.weight clique " << Benchmark::seconds_from_start()
//<< " wallclock seconds" << endl; ~ return clique;
//~ }

template <class Vertex>
typename Graph<Vertex>::SubGraphs Graph<Vertex>::find_fused_rings() {
  SubGraphs fused;
  SubGraphs cycles = find_cycles_connected_graph();
  bool mergeable = true;
  // merge cycles until no more can be merged
  while (mergeable) {
    mergeable = false;
    for (typename SubGraphs::iterator it = cycles.begin(); it != cycles.end();
         ++it) {
      const VertexSet &first = *it;
      VertexSet current(first.begin(), first.end());
      typename SubGraphs::iterator it2 = it;
...
template <class Vertex>
typename Graph<Vertex>::SubGraphs Graph<Vertex>::find_rings() {
  SubGraphs rings;
  SubGraphs cycles = find_cycles_connected_graph();
  ...
  for (typename vector<VertexSet>::iterator it = v.begin(); it != v.end();
    ...
    set_intersection(cycle.begin(), cycle.end(), mapped.begin(), mapped.end(),
...
template <class Vertex>
void Graph<Vertex>::__expand(Vertex &v, Path &path, SubGraphs &cycles,
                             VertexSet &visited) {
  path.push_back(&v);
  visited.insert(&v);
  dbgmsg("new vertex");
...
template <class Vertex>
typename Graph<Vertex>::SubGraphs Graph<Vertex>::find_cycles_connected_graph() {
  Path path;
  SubGraphs cycles;
  VertexSet visited;
...
template <class Vertex>
void Graph<Vertex>::__expand_components(Vertex &v, VertexSet &component,
                                        VertexSet &visited) {
  dbgmsg("new vertex");
  visited.insert(&v);
  component.insert(&v);
  for (auto &adj_v : v) {
    if (!visited.count(&adj_v)) {
      __expand_components(adj_v, component, visited);
    }
  }
}

template <class Vertex>
typename Graph<Vertex>::SubGraphs Graph<Vertex>::find_components() {
  SubGraphs components;
  VertexSet visited;
  for (auto &vertex : *this) {
    if (!visited.count(&vertex)) {
      dbgmsg("expanding the vertex to find disconnected components");
      VertexSet component;
      __expand_components(vertex, component, visited);
...
template <class Vertex> string Graph<Vertex>::get_smiles() const {
  stringstream ss;
  map<Vertex *, int> idx;
  for (auto &v : *this)
    ss << v.get_label() << " ";
...
template <class P> ostream &operator<<(ostream &stream, const Graph<P> &g) {
  map<const P *, int> idx;
  int i = 0;
  for (auto &vertex : g)
    idx[&vertex] = i++;
  ...
  if (!g.__conn.empty()) {
    ...
    for (auto &v1 : g) {
      for (auto &v2 : g) {
        //~ if (g.__conn->size() > 0 && (*g.__conn)[idx[&v1]][idx[&v2]] == true)
        //edge_size++;
        if (g.__conn[idx[&v1]][idx[&v2]] == true)
          edge_size++;
      }
    }
    ...
    for (int i = 0; i < g.size(); i++) {
      for (int j = i + 1; j < g.size(); j++) {
        //~ if (g.__conn->size() > 0 && (*g.__conn)[i][j] == true) stream << "e
        //" << i + 1 << " " << j + 1 << endl;
